...
class Empleado
{
    ...
    public static function ModificarPorId($empleado)
    {
        $accesoDatos=AccesoDatos::DameUnObjetoAcceso();
        $consulta=$accesoDatos->RetornarConsulta('UPDATE empleados SET nombre=:nombre, apellido=:apellido, turno=:turno, clave=:clave, mail=:mail, perfil=:perfil, fechaCreacion=:fechaCreacion, foto=:foto WHERE id=:id');
        ...
        $consulta->bindvalue(':clave', $empleado->clave,PDO::PARAM_LOB  );
        $consulta->bindvalue(':mail', $empleado->mail,PDO::PARAM_STR  );
        $consulta->bindvalue(':perfil', $empleado->perfil,PDO::PARAM_STR  );
        $consulta->bindvalue(':fechaCreacion', $empleado->fechaCreacion,PDO::PARAM_STR  );
        $consulta->bindvalue(':foto', $empleado->foto,PDO::PARAM_STR  );
        $consulta->execute();
    ...
    public static function BorrarUnEmpleado($id)
    {
        $accesoDatos=AccesoDatos::DameUnObjetoAcceso();
